...
export default async function SuperAdminAcademiesPage() {
  ...
  const academies = await Academies.all(true);
  ...
  return (
    ...
    >
      ...
      <p className="text-gray-500 text-sm mb-8">{academies.length} academies (including deleted).</p>

      <div className="space-y-4">
        {academiesWithStats.map(({ academy: a, plan, learners, courses }) => {
          return (
            <div key={a.id} className={`app-card p-6 ${a.is_deleted ? "opacity-60" : ""}`}>
              <div className="flex items-start justify-between flex-wrap gap-4 mb-4">
                <div>
                  <div className="font-bold text-gray-900 text-lg">
                    {a.name} {a.is_deleted ? <span className="text-xs text-red-600 font-semibold ml-2">DELETED</span> : null}
                  </div>
