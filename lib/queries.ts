import { queryAll, queryOne, exec, newId, nowIso, withTransaction } from "./db";
...
export interface Template {
  ...
  // NULL = one of the built-in presets, available to every academy. Set =
  // a custom template a specific academy created for its own branding.
...
export interface SubscriptionPlan {
  ...
  is_active: number;
  sort_order: number;
  // Set once this plan has been provisioned in Stripe (see lib/stripe.ts's
  // ensureStripePrice) — either lazily on first real subscribe, or eagerly
  // via the super-admin "Sync to Stripe" action. NULL until then.
...
export interface Academy {
  ...
  // Real Stripe billing (see lib/actions/billing.ts). NULL until the
  // academy admin has gone through Stripe Checkout at least once.
...
export interface SiteBlock {
  id: string;
  academy_id: string;
  block_type: "TEXT" | "IMAGE" | "VIDEO" | "NEWS";
  title: string;
...
export const Templates = {
  // Preset templates only (academy_id IS NULL) — used wherever there's no
  // specific academy in context yet (signup, super-admin, seeding).
  // Pass academyId to also include that academy's own custom templates.
  async all(academyId?: string): Promise<Template[]> {
    if (academyId) {
      return queryAll<Template>(
        "SELECT * FROM templates WHERE academy_id IS NULL OR academy_id = $1 ORDER BY (academy_id IS NOT NULL), name",
        [academyId]
      );
    }
    return queryAll<Template>("SELECT * FROM templates WHERE academy_id IS NULL ORDER BY name");
  },
  async byId(id: string): Promise<Template | undefined> {
    return queryOne<Template>("SELECT * FROM templates WHERE id = $1", [id]);
  },
  async listCustomByAcademy(academyId: string): Promise<Template[]> {
    return queryAll<Template>("SELECT * FROM templates WHERE academy_id = $1 ORDER BY name", [academyId]);
  },
  async createCustom(input: {
    ...
    const id = newId();
    // Presets use short human keys ("navy-gold" etc); custom templates are
    // scoped to one academy so a random suffix is enough to keep the
    // required-unique `key` column collision-free.
    const key = `custom-${id}`;
    await exec(
      `INSERT INTO templates (id, key, name, description, primary_color, secondary_color, accent_color, font_heading, preview_style, academy_id)
    ...
    return (await Templates.byId(id))!;
  },
  // Only ever deletes a template that (a) belongs to this academy and (b) is
  // actually a custom one — academy_id = $2 already rules out presets, since
  // presets always have academy_id NULL.
  async removeCustom(id: string, academyId: string): Promise<void> {
    await exec("DELETE FROM templates WHERE id = $1 AND academy_id = $2", [id, academyId]);
  },
};
...
export const Plans = {
  ...
  async setStripeIds(id: string, stripeProductId: string, stripePriceId: string): Promise<void> {
    await exec("UPDATE subscription_plans SET stripe_product_id=$1, stripe_price_id=$2, updated_at=$3 WHERE id=$4", [
      stripeProductId,
      stripePriceId,
      nowIso(),
      id,
...
export const Academies = {
  ...
  // Irreversibly erases an academy and everything belonging to it — the
  // "Delete Permanently" action available only from the Deleted academies
  // section, once an academy has already been soft-deleted (see
  // softDelete/restore above). Unlike softDelete, there's no undo after
  // this.
  //
  // Most academy-owned tables reference academies(id) *without* ON DELETE
  // CASCADE (users, courses, modules, enrollments, payments,
  // subscription_invoices) — deliberately, so an accidental
  // `DELETE FROM academies` elsewhere in the app fails loudly with a
  // foreign key error instead of silently wiping tenant data. That means a
  // real permanent delete has to remove dependent rows itself, in an order
  // that respects every foreign key — leaf tables first, then courses/
  // modules, then users, then the academy row itself — all inside one
  // transaction so a failure partway through can't leave the academy's
  // data half-erased.
  async permanentlyDelete(id: string): Promise<void> {
    await withTransaction(async (run) => {
      // Cascades (via ON DELETE CASCADE) to quiz_questions, quiz_options,
      // and any quiz_attempts tied to these quizzes.
      await run("DELETE FROM quizzes WHERE academy_id = $1", [id]);
      // Cascades to scorm_attempts.
      await run("DELETE FROM scorm_packages WHERE academy_id = $1", [id]);
      await run("DELETE FROM certificates WHERE academy_id = $1", [id]);
      await run("DELETE FROM academy_integrations WHERE academy_id = $1", [id]);
      await run("DELETE FROM site_blocks WHERE academy_id = $1", [id]);
      await run("DELETE FROM templates WHERE academy_id = $1", [id]); // academy's own custom templates only (built-in presets have academy_id NULL)
      await run("DELETE FROM payments WHERE academy_id = $1", [id]);
      await run("DELETE FROM subscription_invoices WHERE academy_id = $1", [id]);
      // Cascades to module_completions.
      await run("DELETE FROM enrollments WHERE academy_id = $1", [id]);
      // Cascades to course_modules and course_instructors.
      await run("DELETE FROM courses WHERE academy_id = $1", [id]);
      // Cascades to any remaining module_instructors.
      await run("DELETE FROM modules WHERE academy_id = $1", [id]);
      // Cascades to password_reset_tokens. Safe now — every other table
      // that pointed at these users (courses.created_by, enrollments,
      // payments, quiz/scorm attempts, certificates, academy_integrations)
      // has already been cleared above.
      await run("DELETE FROM users WHERE academy_id = $1", [id]);
      await run("DELETE FROM academies WHERE id = $1", [id]);
    });
  },
  // Only ever returns an academy whose domain has actually passed DNS
  // ownership verification — an unverified custom_domain value alone (set
  // but never proven) must not route traffic.
  async byCustomDomain(domain: string): Promise<Academy | undefined> {
    return queryOne<Academy>(
      "SELECT * FROM academies WHERE custom_domain = $1 AND custom_domain_verified_at IS NOT NULL AND is_deleted = 0",
      [domain.toLowerCase()]
    );
  },
  async setCustomDomain(id: string, domain: string, token: string): Promise<void> {
    // Changing the domain always resets verification — a new domain hasn't
    // been proven yet, even if a previous one on this academy was.
    await exec(
      "UPDATE academies SET custom_domain=$1, custom_domain_verification_token=$2, custom_domain_verified_at=NULL, updated_at=$3 WHERE id=$4",
      [domain.toLowerCase(), token, nowIso(), id]
    );
  },
  async markCustomDomainVerified(id: string): Promise<void> {
    await exec("UPDATE academies SET custom_domain_verified_at=$1, updated_at=$1 WHERE id=$2", [nowIso(), id]);
  },
  async removeCustomDomain(id: string): Promise<void> {
    await exec(
      "UPDATE academies SET custom_domain=NULL, custom_domain_verification_token=NULL, custom_domain_verified_at=NULL, updated_at=$1 WHERE id=$2",
      [nowIso(), id]
    );
  },
  async byStripeCustomerId(stripeCustomerId: string): Promise<Academy | undefined> {
    return queryOne<Academy>("SELECT * FROM academies WHERE stripe_customer_id = $1", [stripeCustomerId]);
  },
  async setStripeCustomer(id: string, stripeCustomerId: string): Promise<void> {
    await exec("UPDATE academies SET stripe_customer_id=$1, updated_at=$2 WHERE id=$3", [stripeCustomerId, nowIso(), id]);
  },
  // Called from the Stripe webhook once a Checkout Session or subscription
  // event confirms real payment — updates the plan/status together so an
  // academy that changed plans mid-checkout ends up on the plan it actually
  // paid for, not whatever it had before.
  async setStripeSubscription(
    id: string,
    input: { stripeSubscriptionId: string; subscriptionStatus: string; planId?: string }
  ): Promise<void> {
    const current = await Academies.byId(id);
    if (!current) return;
    const plan_id = input.planId ?? current.plan_id;
    await exec(
      "UPDATE academies SET stripe_subscription_id=$1, subscription_status=$2, plan_id=$3, updated_at=$4 WHERE id=$5",
      [input.stripeSubscriptionId, input.subscriptionStatus, plan_id, nowIso(), id]
    );
  },
};
...
export const SiteBlocks = {
  async byId(id: string): Promise<SiteBlock | undefined> {
    return queryOne<SiteBlock>("SELECT * FROM site_blocks WHERE id = $1", [id]);
  },
  // publishedOnly=true is what the live public site uses; the admin editor
  // passes false so it can also show/manage blocks that are toggled off.
  async listByAcademy(academyId: string, publishedOnly = false): Promise<SiteBlock[]> {
    const sql = publishedOnly
      ? "SELECT * FROM site_blocks WHERE academy_id = $1 AND is_published = 1 ORDER BY order_index ASC"
      : "SELECT * FROM site_blocks WHERE academy_id = $1 ORDER BY order_index ASC";
    return queryAll<SiteBlock>(sql, [academyId]);
  },
  async create(input: {
    academy_id: string;
    block_type: "TEXT" | "IMAGE" | "VIDEO" | "NEWS";
    ...
    const id = newId();
    const now = nowIso();
    const existing = await SiteBlocks.listByAcademy(input.academy_id);
    await exec(
      `INSERT INTO site_blocks (id, academy_id, block_type, title, body_text, image_path, video_url, is_published, order_index, created_at, updated_at)
    ...
    return (await SiteBlocks.byId(id))!;
  },
  async update(
    id: string,
    patch: Partial<{
  ...
  ): Promise<SiteBlock | undefined> {
    const current = await SiteBlocks.byId(id);
    if (!current) return undefined;
    const title = patch.title ?? current.title;
    const body_text = patch.body_text !== undefined ? patch.body_text : current.body_text;
    const image_path = patch.image_path !== undefined ? patch.image_path : current.image_path;
    const video_url = patch.video_url !== undefined ? patch.video_url : current.video_url;
    const is_published = patch.is_published !== undefined ? (patch.is_published ? 1 : 0) : current.is_published;
    await exec(
      `UPDATE site_blocks SET title=$1, body_text=$2, image_path=$3, video_url=$4, is_published=$5, updated_at=$6 WHERE id=$7`,
      [title, body_text, image_path, video_url, is_published, nowIso(), id]
    );
    return SiteBlocks.byId(id);
  },
  async remove(id: string): Promise<void> {
    await exec("DELETE FROM site_blocks WHERE id = $1", [id]);
  },
  async move(id: string, direction: "up" | "down"): Promise<void> {
    const block = await SiteBlocks.byId(id);
    if (!block) return;
    const siblings = await SiteBlocks.listByAcademy(block.academy_id);
    const idx = siblings.findIndex((b) => b.id === id);
    const swapIdx = direction === "up" ? idx - 1 : idx + 1;
    if (idx === -1 || swapIdx < 0 || swapIdx >= siblings.length) return;
    const other = siblings[swapIdx];
    await exec("UPDATE site_blocks SET order_index=$1, updated_at=$2 WHERE id=$3", [other.order_index, nowIso(), block.id]);
    await exec("UPDATE site_blocks SET order_index=$1, updated_at=$2 WHERE id=$3", [block.order_index, nowIso(), other.id]);
  },
};
...
export const Users = {
  ...
  async setPassword(id: string, password_hash: string): Promise<void> {
    await exec("UPDATE users SET password_hash = $1 WHERE id = $2", [password_hash, id]);
  },
  async remove(id: string): Promise<void> {
...
export const PasswordResetTokens = {
  async create(userId: string, tokenHash: string, expiresAt: string): Promise<PasswordResetToken> {
    const id = newId();
    await exec(
    ...
    return (await queryOne<PasswordResetToken>("SELECT * FROM password_reset_tokens WHERE id = $1", [id]))!;
  },
  async byTokenHash(tokenHash: string): Promise<PasswordResetToken | undefined> {
    return queryOne<PasswordResetToken>("SELECT * FROM password_reset_tokens WHERE token_hash = $1", [tokenHash]);
  },
  async markUsed(id: string): Promise<void> {
    await exec("UPDATE password_reset_tokens SET used_at = $1 WHERE id = $2", [nowIso(), id]);
  },
  // Called when a new reset is requested, so an old unused link from an
  // earlier request can't still be used to reset the password after a
  // newer one was issued.
  async invalidateAllForUser(userId: string): Promise<void> {
    await exec("UPDATE password_reset_tokens SET used_at = $1 WHERE user_id = $2 AND used_at IS NULL", [nowIso(), userId]);
  },
};
...
export const Modules = {
  ...
  async update(
    ...
    const current = await Modules.byId(id);
    if (!current) return undefined;
    const title = patch.title ?? current.title;
    const description = patch.description ?? current.description;
    const content_text = patch.content_text !== undefined ? patch.content_text : current.content_text;
    const content_url = patch.content_url !== undefined ? patch.content_url : current.content_url;
    const file_path = patch.file_path !== undefined ? patch.file_path : current.file_path;
    const file_name = patch.file_name !== undefined ? patch.file_name : current.file_name;
    const file_mime = patch.file_mime !== undefined ? patch.file_mime : current.file_mime;
    const file_size = patch.file_size !== undefined ? patch.file_size : current.file_size;
    const live_provider = patch.live_provider !== undefined ? patch.live_provider : current.live_provider;
    const live_join_url = patch.live_join_url !== undefined ? patch.live_join_url : current.live_join_url;
    const live_start_time = patch.live_start_time !== undefined ? patch.live_start_time : current.live_start_time;
    const live_duration_minutes =
      patch.live_duration_minutes !== undefined ? patch.live_duration_minutes : current.live_duration_minutes;
    const live_password = patch.live_password !== undefined ? patch.live_password : current.live_password;
    await exec(
      `UPDATE modules SET title=$1, description=$2, content_text=$3, content_url=$4, file_path=$5, file_name=$6, file_mime=$7, file_size=$8, live_provider=$9, live_join_url=$10, live_start_time=$11, live_duration_minutes=$12, live_password=$13, updated_at=$14 WHERE id=$15`,
      [
        title,
        description,
        content_text,
    ...
    return Modules.byId(id);
  },
  // setLiveMeeting() was removed — it existed only to save the meeting
  // details returned by the old OAuth-based Zoom/Microsoft API calls
  // (live_meeting_id, live_start_url as a "start as host" link). Now that
  // admins/instructors just paste their own meeting link, Modules.create()
  // is the only place live_* fields get written.
  async remove(id: string): Promise<void> {
...
export interface SubscriptionInvoice {
  ...
  period_start: string;
  period_end: string;
  // 'simulated' for the legacy fake checkout rows recorded by earlier
  // passes; 'stripe' for rows the webhook created from a real charge.
  provider: string;
...
export const SubscriptionInvoices = {
  async create(input: {
    ...
    const id = newId();
    const start = input.period_start ?? nowIso();
    const end = input.period_end ?? new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString();
    await exec(
      `INSERT INTO subscription_invoices (id, academy_id, plan_id, amount_pence, period_start, period_end, provider, stripe_invoice_id, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
      [id, input.academy_id, input.plan_id, input.amount_pence, start, end, input.provider ?? "simulated", input.stripe_invoice_id ?? null, nowIso()]
    );
  ...
  async byStripeInvoiceId(stripeInvoiceId: string): Promise<SubscriptionInvoice | undefined> {
    return queryOne<SubscriptionInvoice>("SELECT * FROM subscription_invoices WHERE stripe_invoice_id = $1", [stripeInvoiceId]);
  },
  async totalCollected(provider = "stripe"): Promise<number> {
    const row = await queryOne<{ total: string | null }>(
      "SELECT SUM(amount_pence) as total FROM subscription_invoices WHERE provider = $1 AND status = 'paid'",
      [provider]
    );
    return row?.total ? parseInt(row.total, 10) : 0;
  },
};

// Note: the `academy_integrations` and `oauth_states` tables still exist in
// the database schema (see lib/db.ts) but are no longer used — this app
// used to let academies OAuth-connect a Zoom/Microsoft account so it could
// create meetings via their APIs. That's been replaced by academy admins
// and instructors simply pasting their own meeting link when creating a
// LIVE_SESSION module (see lib/actions/modules.ts) — no stored credentials
// or API calls involved. The tables were left in place rather than dropped
// to avoid a destructive migration against production data; they're just
// dead weight now and safe to ignore (or drop in a future migration).
...
// Stripe retries a webhook delivery on anything but a 2xx response, and
// can also occasionally deliver the same event twice even without a
// retry. markProcessed relies on stripe_event_id's UNIQUE constraint: the
// first caller to insert a given event id gets `true` (go ahead and
// process it); every later attempt at the same event id hits a conflict
// and gets `false` (already handled, skip).
export const StripeWebhookEvents = {
  async markProcessed(stripeEventId: string, eventType: string): Promise<boolean> {
    const id = newId();
    try {
      await exec("INSERT INTO stripe_webhook_events (id, stripe_event_id, event_type, created_at) VALUES ($1, $2, $3, $4)", [
        id,
        stripeEventId,
        eventType,
        nowIso(),
