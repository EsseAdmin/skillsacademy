...
## Alternative: deploying to Render instead

A `render.yaml` blueprint is still included in this repo from an earlier
iteration of this project, for a Render deploy. It's stale now — it was
written for the local-SQLite-file version of this app, before the Postgres
+ Netlify Blobs migration, and would need `DATABASE_URL` (pointed at a
Postgres add-on) and Netlify Blobs swapped for equivalents Render supports
(e.g. Render's own managed Postgres, and an S3-compatible bucket for file
storage) to work as-is. Netlify is the actively maintained deployment path
for this app going forward; keep `render.yaml` only if you specifically
want to compare hosts later.
